...
def get_forecast(latitude="60.1705802", longitude="24.9428433"):
    #returns array composed of pairs. for example [['freezing', 40], ['wet', 30]]. Where the first position of the pair is the weather state, and the second is the duration of that state.
    ...
    if currently["time"] - hourly["data"][0]["time"] < 600: # very close to value, we don't bother with what comes up after
        ...
        if parse_code(curr_aux) == parse_code(hour_aux):
            ...
            for x in range(change+1):
                ...
                if init_weight > weight[parse_code(hour_aux)]:
                    init_weight-=1
                elif init_weight < weight[parse_code(hour_aux)]:
                    init_weight+=1
            return result
        ...
        if parse_code(curr_aux) == parse_code(hour_aux) == parse_code(hour_aux1):
            ...
            aux_time = 60-(hourly["data"][1]["time"]-currently["time"])/60
            result.append([parse_code(curr_aux), (60-aux_time + (aux_time/(change+1)))])
            result.append([parse_code(hour_aux1), (aux_time/(change+1))])
            return result
            #extreme case ignored where a change bigger than 1 would happen
        elif parse_code(hour_aux) == parse_code(hour_aux1): # change stabilizes
            result = []
            change = abs(weight[parse_code(hour_aux)] - weight[parse_code(curr_aux)])
            aux_time = 60-(hourly["data"][1]["time"]-currently["time"])/60
            result.append([parse_code(curr_aux), (aux_time/(change+1))])
            result.append([parse_code(hour_aux1), (60-aux_time + (aux_time/(change+1)))])
            return result
            #extreme case ignored where a change bigger than 1 would happen
        else: # two changes (not likely)
            # really extreme cases where a change greater than 1 will happen are ignored
            result = []
            aux_time1 = (hourly["data"][1]["time"] - currently["time"])/60
            aux_time2 = 60 - aux_time1
            result.append([parse_code(curr_aux), aux_time1/2])
            result.append([parse_code(hour_aux), (aux_time1/2)+(aux_time2/2)])
            result.append([parse_code(hour_aux1), (aux_time2/2)])
            return result
